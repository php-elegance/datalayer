...
abstract class Connection
{
    ...
    {
        $this->dbName = Datalayer::format_dbName($dbName);
    ...
    function map(?bool $type = null): array
    {
        if (is_null($type)) {
            ...
            foreach ($realMap as $tableName => $realTable) {
                ...
                $map[$tableName] = [
                    'ref' => $registrdTable['ref'] ?? Datalayer::format_tableName($tableName, true),
                ...
                foreach ($realTable['fields'] as $fieldName => $realField) {
                    ...
                    $map[$tableName]['fields'][$fieldName] = [
                        'ref' => $registredField['ref'] ?? Datalayer::format_fieldName($fieldName, true),
