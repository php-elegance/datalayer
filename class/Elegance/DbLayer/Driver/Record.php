...
use Elegance\DbLayer\Driver\Field\FIdx;
...
abstract class Record
{
    ...
    final function _setArray(array $scheme): static
    {
        foreach ($scheme as $name => $value) {
            $name = DbLayer::format_fieldName($name, true);
    ...
    {
        $this->__id = Query::insert(DbLayer::format_tableName($this->__tableRef))
    ...
    final protected function __runUpdate()
    {
        ...
        if (!empty($dif)) {
            Query::update(DbLayer::format_tableName($this->__tableRef))
                ->where('id', $this->__id)
                ->values($dif)
                ->run($this->__dbName);
        }
    ...
    {
        Query::delete(DbLayer::format_tableName($this->__tableRef))
            ->where('id', $this->__id)
            ->limit(1)
            ->run($this->__dbName);
