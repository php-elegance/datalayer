...
namespace Elegance\DbLayer\Scheme;
...
class SchemeMap
{
    ...
    {
        $this->dbName = $dbName;
        $this->map = DbLayer::get($this->dbName)->map();
        $this->realMap = DbLayer::get($this->dbName)->map(true);
    }
    ...
    function save(): void
    {
        DbLayer::get($this->dbName)->config('elegance_dbmap', $this->map);
    ...
    {
        $sqlTableName = DbLayer::format_tableName($tableName, true);
        $sqlFieldName = DbLayer::format_tableName($fieldName, true);

        return $this->getTable($tableName, $inRealMap)['fields'][$sqlFieldName] ?? [
            'ref' => DbLayer::format_fieldName($fieldName),
            ...self::FIELD_MAP
        ];
    }
    ...
    function getTable(string $tableName, bool $inRealMap = false): array
    {
        ...
        return $this->get($inRealMap)[$sqlTableName] ?? [
            'ref' => DbLayer::format_tableName($tableName),
            ...self::TABLE_MAP
        ];
    }
