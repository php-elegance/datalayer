...
trait TraitMigration
{
    ...
    static function up($dbName = null)
    {
        ...
        if (!$result)
            self::echo('Todas as mudanças foram aplicadas');
    ...
    static function down($dbName = null)
    {
        ...
        if (!$result)
            self::echo('Todas as mudanças foram revertidas');
    ...
    protected static function getFiles(): array
    {
        ...
        foreach (Dir::seek_for_file(self::$path) as $file)
            if (substr($file, -4) == '.php')
                $files[substr($file, 1, 10)] = self::$path . "/$file";
    ...
    {
        self::echo("[#action] migration [#file]", [
